...
//* Crear una función que reciba 2 (dos) parámetros: un arreglo y un string, en el cual,
//* el primer parámetro hará referencia al arreglo con el formato del arreglo "empresas" definido
...
const nomEmp = "Empresa2"
  
  function verificarEmpleados(empresas, nomEmp) {
    let empFind = false;
    let total = 0;
  
    empresas.forEach((empresa) => {
      if (empresa.nombre === nomEmp) {
        empFind = true;
        for (let local in empresa.informacion.locales) {
          total += empresa.informacion.locales[local].empleados;
        }
      }
    });
    ...
    buscarEmpresa = empresas.find((empresa) => empresa.nombre === nomEmp).informacion.empleados;
    
    if (buscarEmpresa==total){
        return true
    }
  ...
  const resultado = verificarEmpleados(empresas, nomEmp);
